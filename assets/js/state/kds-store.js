(function (window) {
    ...
    CoffeePOS.state.createKdsStore = function () {
        ...
        return {
            ...
            accept: function (data) { state.orders = sortOrders(data.orders); state.serverOffset = Date.parse(data.server_time || '') - Date.now(); if (!Number.isFinite(state.serverOffset)) { state.serverOffset = 0; } state.loaded = true; state.error = ''; },
            ...
            replace: function (order) { const id = Number(order && order.id); state.orders = state.orders.filter(function (item) { return Number(item.id) !== id; }); if (order && ['new', 'preparing', 'ready'].indexOf(order.kds.state) !== -1) { state.orders.push(order); } state.orders = sortOrders(state.orders); },
