(function (window) {
    ...
    CoffeePOS.state.createOrderQueueStore = function () {
        ...
        return {
            ...
            accept: function (data) { state.orders = Array.isArray(data.orders) ? data.orders : []; state.loaded = true; state.error = ''; },
            ...
            replace: function (order) { const id = Number(order && order.id); state.orders = state.orders.filter(function (item) { return Number(item.id) !== id; }); if (order) { state.orders.push(order); } },
